...
Transforme un enregistrement audio `.wav` d'un passage satellite NOAA (15, 18 ou 19) en image BMP exploitable, en reproduisant l'intégralité du pipeline de traitement du signal : démodulation AM, synchronisation par corrélation, rééchantillonnage, et post-traitement.
...
- **Export BMP** : écriture directe au format BMP 8-bit niveaux de gris, sans dépendance externe
...
Nécessite `gcc` et `make`. Aucune bibliothèque externe (uniquement la libc et libm).
...
│   ├── bmp.c          # Encodeur BMP 8-bit avec palette de gris
│   └── enhance.c      # Étirement de contraste + filtre médian
...
```
WAV 11025 Hz → Redressement → Lissage → Pixels 0-255 → Sync NCC → Lignes 2080 px → BMP
```
...
4. **enhance.c** : étirement de contraste par percentiles 2%-98%, filtre médian 3x3 par tri insertion
5. **bmp.c** : écrit l'image en BMP 8-bit avec palette de gris 256 couleurs
